    ...
    def __init__(self):
		...
		rospy.loginfo("Topic is = %s", topic)
                if topic == "/camera/color/image_raw":
                    ...
                    image_name = str(save_dir)+"/color_"+timestr+".pgm"
                    #image_name = str(save_dir)+"/"+timestr+"_left"+".pgm"
                    cv2.imwrite(image_name, cv_image)
		    rospy.loginfo("Image saved in topic color")
                ...
                '''if topic == "/camera/depth/image_raw":
                    try:
                        ...
                        print e
                    timestr = "%.6f" % msg.header.stamp.to_sec()
                    image_name = str(save_dir)+"/depth_"+timestr+".pgm"
                    #image_name = str(save_dir)+"/"+timestr+"_right"+".pgm"
                    cv2.imwrite(image_name, cv_image)
		    rospy.loginfo("Image saved in topic depth")'''
